...
function loadProgress(): Partial<TestState> {
    ...
    try {
        ...
        if (data) {
            ...
            return { completedTests: parsed.completedTests || [], streak: parsed.streak || 0, totalQuestionsAnswered: parsed.totalQuestionsAnswered || 0 };
        }
    } catch { }
    return {};
}
...
function saveProgress(state: Partial<TestState>) {
    if (typeof window === 'undefined') return;
    try {
        localStorage.setItem('targetprep_progress', JSON.stringify({
...
export const useTestStore = create<TestStore>((set, get) => ({
    ...
    endTest: (results) => {
        ...
        const newState = {
            ...
            streak: s.streak + 1,
            totalQuestions: s.totalQuestionsAnswered + results.totalQuestions,
